...
const fmtISO = x => { if(!x) return ''; if(/^\d{4}-\d{2}-\d{2}$/.test(x)) return x; const d=new Date(x); if(isNaN(d)) return x; return `${d.getFullYear()}-${pad2(d.getMonth()+1)}-${pad2(d.getDate())}`; };
...
/* Signature pad */
class SigPad{
  constructor(canvas){ this.canvas=canvas; this.ctx=canvas.getContext('2d'); this.drawing=false; this.resize();
    window.addEventListener('resize',()=>this.resize());
    canvas.addEventListener('pointerdown',e=>this.start(e)); canvas.addEventListener('pointermove',e=>this.move(e));
    canvas.addEventListener('pointerup',e=>this.end(e)); canvas.addEventListener('pointerleave',e=>this.end(e)); }
  resize(){ const r=this.canvas.getBoundingClientRect(), dpr=window.devicePixelRatio||1; this.canvas.width=r.width*dpr; this.canvas.height=r.height*dpr; this.ctx.setTransform(1,0,0,1,0,0); this.ctx.scale(dpr,dpr); this.ctx.lineWidth=2; this.ctx.lineCap='round'; this.ctx.strokeStyle='#111'; }
...
async function initIndex(){
  ...
  const sig=new SigPad($('#firmaLimpieza')); fillMatriculas(matricula); fecha.value=nowISO();
...
async function initReview(){
  const id=qs('id'); if(!id){ alert('Falta ID'); return; }
  const res=await fetch(WEB_APP_URL+'?action=getRecord&id='+encodeURIComponent(id)); const data=await res.json();
